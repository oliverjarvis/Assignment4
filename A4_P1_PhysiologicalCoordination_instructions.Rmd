...
removeOutsBinary <- function(ts){
  ...
  print(withinThresh[length(withinThresh)])
  if(withinThresh[length(withinThresh)] < ts[length(ts)]){
...
# plots plots plots
ggplot(data=output, aes(time, HR1)) + 
  geom_path(aes(group=pairID,  color=trial))
# Remove bad data
